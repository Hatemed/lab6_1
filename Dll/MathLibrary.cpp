...
void vmd(int n, double min, double max, int num_func, int time[], double max_value[])
{
    ...
    double* value_base = new double[n];
    ...
    if (num_func == 0) {
        ...
        start_time = clock();
        for (int i = 0; i < n; i++) {
            value_base[i] = my_erfinvf(args[i]);
        }
        end_time = clock();
        time[2] = end_time - start_time;
    }
